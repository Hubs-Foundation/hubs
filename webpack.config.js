...
async function fetchAppConfigAndEnvironmentVars() {
  ...
  // dev.reticulum.io.invalid doesn't run ita
  if (host === "dev.reticulum.io.invalid") {
    return appConfig;
  }
  ...
  if (argv.mode !== "production" || env.bundleAnalyzer) {
    if (env.loadAppConfig || process.env.LOAD_APP_CONFIG) {
      if (!env.localDev) {
        ...
        // A Hubs Cloud server or dev.reticulum.io.invalid can be used.
    ...
    if (env.localDev) {
      ...
      Object.assign(process.env, {
        ...
        NON_CORS_PROXY_DOMAINS: `${localDevHost},dev.reticulum.io.invalid`,
