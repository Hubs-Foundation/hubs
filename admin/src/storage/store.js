...
const defaultMaterialQuality = (function () {
  ...
  if (isMobile) {
    ...
    if (
      qsMobileDefault &&
      MATERIAL_QUALITY_OPTIONS.indexOf(qsMobileDefault) !== -1
    ) {
...
const isFirefoxReality =
  window.AFRAME?.utils.device.isMobileVR() &&
    ...
    preferences: {
      ...
      properties: {
        ...
        materialQualitySetting: {
          type: 'string',
          default: defaultMaterialQuality,
        },
        ...
        audioPanningQuality: {
          type: 'string',
          default: defaultAudioPanningQuality(),
        },
...
export default class Store extends EventTarget {
    ...
    const maybeDispatchThemeChanged = (() => {
      ...
      return () => {
        ...
        if ((previous || current) && previous !== current) {
          this.dispatchEvent(
            new CustomEvent('themechanged', { detail: { current, previous } })
          );
        }
        previous = current;
      };
  ...
  get state() {
    if (!Object.prototype.hasOwnProperty.call(this, STORE_STATE_CACHE_KEY)) {
      const state = (this[STORE_STATE_CACHE_KEY] = JSON.parse(
      ...
      for (const [key, props] of Object.entries(
        SCHEMA.definitions.preferences.properties
      )) {
  ...
  update(newState, mergeOpts) {
    const finalState = merge(
      { ...this.state, preferences: this._preferences },
      newState,
      mergeOpts
    );
    ...
    if (!valid) {
      errors.forEach((error) => {
        console.error(
          `Removing invalid preference from store: ${error.message}`
        );
  ...
  getEmbedTokenForHub(hub) {
    const embedTokenEntry = this.state.embedTokens.find(
      (embedTokenEntry) => embedTokenEntry.hubId === hub.hub_id
    );
    if (embedTokenEntry) {
