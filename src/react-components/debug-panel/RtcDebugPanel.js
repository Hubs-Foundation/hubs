...
function TrackStatsPanel({ title, data, stats }) {
  const props = {
    id: data.id,
    opened: data.opened,
    paused: data.paused,
...
export default class RtcDebugPanel extends Component {
  ...
  createProducers(producers, stats) {
    ...
    producers?.forEach(producer => {
      components.push(
        <TrackStatsPanel key={producer.id} title={"producer"} data={producer} stats={stats[producer.id]} />
      );
    });
  ...
  createConsumers(consumers, stats) {
    ...
    for (const reducedConsumerId in reducedConsumers) {
      ...
      for (const consumerId in reducedConsumer.consumers) {
        ...
        panels.push(
          <TrackStatsPanel key={consumer.id} title={"consumer"} data={consumer} stats={stats[consumer.id]} />
        );
      }
      components.push(
        <CollapsiblePanel
