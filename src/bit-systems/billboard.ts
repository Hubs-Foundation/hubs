...
const updateIsInView = (world: HubsWorld, billboard: number): boolean => {
  const object3D = world.eid2obj.get(billboard)!;
  if (!object3D.visible || !APP.camera) {
    return false;
  }

  const inVR = APP.scene?.is("vr-mode");
  return inVR || isInViewOfCamera(object3D, APP.camera);
};

const updateBillboard = (world: HubsWorld, billboard: number) => {
  const object3D = world.eid2obj.get(billboard)!;
...
export function billboardSystem(world: HubsWorld) {
  const billboards = billboardQuery(world);
  if (!billboards.length) return;
  if (!isMobileVR) {
    billboards.forEach(billboard => updateBillboard(world, billboard));
  } else {
    if (nextBillboard >= billboards.length) {
      nextBillboard = 0;
    }
    const billboard = billboards[nextBillboard++];
    updateIsInView(world, billboard) && updateBillboard(world, billboard);
  }
}
