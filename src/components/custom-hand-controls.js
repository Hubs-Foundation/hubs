...
AFRAME.registerComponent("custom-hand-controls", {
  ...
  update: function(prevData) {
    ...
    const controlConfiguration = {
      hand: hand,
      model: false,
      rotationOffset: 0 //hand === "left" ? 90 : -90
    };

    // Set model.
    if (model !== prevData.model) {
      ...
      //el.removeEventListener("model-loaded", onLoad);
      // }
      // el.addEventListener("model-loaded", onLoad);
      // this.loader.load(MODEL_URLS[hand], function(scene) {
      //   const mesh = scene.getObjectByName("Hand");
  ...
  handleButton: function(button, evt) {
    ...
    const lastGesture = this.gesture;
  ...
  */
  playAnimation: function(gesture, lastGesture, reverse) {
    ...
    // Grab clip action.
    const toAction = mesh.mixer.clipAction(gesture);
    ...
    const fromAction = mesh.mixer.clipAction(lastGesture);
    mesh.mixer.stopAllAction();
...
function getGestureEventName(gesture, active) {
  ...
  const eventName = EVENTS[gesture];
  if (eventName === "grip") {
