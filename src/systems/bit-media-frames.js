...
const capturableObjectEnterQuery = enterQuery(capturableQuery);
...
export function mediaFramesSystem(world, physicsSystem) {
  ...
  capturableObjectEnterQuery(world).forEach(eid => {
  ...
  for (let i = 0; i < mediaFrames.length; i++) {
    ...
    if (capturedEid) {
      const capturable = findChildWithComponent(world, Capturable, capturedEid);
      if (MediaFrame.flags[frame] & MEDIA_FRAME_FLAGS.LOCKED) {
        removeComponent(world, Holdable, capturable);
      } else if (!hasComponent(world, Holdable, capturable)) {
        addComponent(world, Holdable, capturable);
      }
    }
